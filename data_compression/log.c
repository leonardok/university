...
#ifdef DEBUG
        int declared_level = LOG_DEBUG;
#endif
#ifdef WARNING
        int declared_level = LOG_WARNING;
#endif
#ifdef INFO
        int declared_level = LOG_INFO;
#endif
#ifdef CRITICAL
        int declared_level = LOG_CRITICAL;
#endif

	/* get out if the global log level is higher than the level of what 
	 * we are writing 
	 */
	if (log_level < declared_level)
		return;
